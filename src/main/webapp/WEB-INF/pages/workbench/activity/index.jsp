...
<script type="text/javascript">
	...
	$(function(){
		...
		$(".mydate").datetimepicker({
			...
			todayBtn:true,//设置是否显示今天按钮，默认是false
		...
		$("#tBody").on("click","input[type='checkbox']",function () {
			...
			}else{//如果列表中所有的checkbox至少一个没选中，则全选按钮也取消
		...
		$("#exportActivityAllBtn").click(function () {
			//发送同步请求
			window.location.href="exportAllActivities"
		});
		//给导入按钮添加单击事件
		$("#importActivityBtn").click(function () {
			...
			$.ajax({
				url:'importActivity',
				data:formData,
				processData:false,//设置ajax向后台提交参数之前，是否把参数同意转换成字符串
				contentType:false,//设置ajax向后台提交参数之前，是否把所有参数统一按urlencoded编码
				type:'post',
				dataType:'json',
				success:function (data) {
					if (data.code=="1"){
	...
	function queryActivityByConditionForPage(pageNo,pageSize) {
		...
		//发送请求
		$.ajax({
			rul:'queryCountOfActivityByCondition',
			data:{
				name:name,
				owner:owner,
				startDate:startDate,
			...
			success:function (data) {
				...
				$.each(data.activityList,function (index,obj) {
					...
					htmlStr+="<td><input type=\"checkbox\" value=\""+obj.name+"\" /></td>";
				...
				$("#demo_page1").bs_pagination({
					...
					visiblePageLinks:totalPages,//最多可以显示的卡片数
					showGoToPage:true,//是否显示跳转到部分，默认true
					showRowsPerPage:true,//是否显示每页显示条数部分，默认true
					showRowsInfo:true,//是否显示记录的信息，默认true
					//用户每次切换页号，都会自动触发本函数
					//每次返回切换也好之后的pageNo和pageSize
